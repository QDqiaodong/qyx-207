...
候车厅急救箱实行**按箱记账**（`first_aid_kit` 档案表 + `station_kit_stock` 本站台账 + `line_kit_stock` 线路台账）：批次和到期日落在每一箱头上，本站可用箱数与按线路加起来的可用箱数分两本台账记，两边永远同一事务一起动——到期的药品不许再按能用计，两边箱数也不许对不上。

- **按箱记批次和到期日**：`POST /api/kits` 建档（`kitCode`、`stationId` 必填；`batchNo`、`expiryDate` 选填但要填一起填），`PUT /api/kits/{kitId}/expiry` 给箱子补写批次和到期日，两样一样都不能少，落笔时刻一并留档。建档时就填了到期日且已经到期的，建档即撤下，不计入可用箱数。
- **到期日到了，两处各减一**：写上的到期日已经到了（今天或更早），箱子当场置为已到期撤下，本站台账与线路台账的可用箱数**同一事务各减一**——只减这一箱，别站、别线的计数一根指头不动。
- **没到期的不从可用里拿掉**：写上将来的到期日，只落批次和到期日，箱子照算可用，两处计数不动。
- **两人抢写只成一次**：两名站上的人抢着给同一箱写到期日，服务端对箱子行加悲观写锁串行——先到的落批次和到期日（到期的连同两处计数一起核减），后到的看到箱上已有到期日被拒，同一箱只成一次，计数不会减两遍。
- **写到一半中断整体回滚**：箱子、本站台账、线路台账任何一步失败（如台账行缺失、计数已为 0 还要核减），整笔回滚——两处可用箱数仍是动手前的数，箱子上也不留半截日期。
- **只改箱子不算做完**：到期撤下时箱子状态字、本站台账、线路台账三处同一事务一起落；只在箱子上改个日期、线路侧箱数不动，不算完成。
- 两处可用箱数可随时对账：`GET /api/kits/stocks` 一次端出本站台账（逐站）与线路台账（逐线），每条线的可用箱数应等于它各站之和；箱子名单走 `GET /api/kits`。
